<div class="space-y-8">
    <div class="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div class="space-y-2">
            ...
            <div>
                ...
                <flux:text class="mt-2 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                    Manage shipping agencies, contact records, operational status, exports, and print-ready reports.
                </flux:text>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            @can('create agencies')
                <flux:button :href="route('agencies.create')" wire:navigate icon="plus">
                    Add Agency
                </flux:button>
            @endcan
            ...
            @can('export agencies')
                <flux:dropdown position="bottom" align="end">
                    ...
                    <flux:menu>
                        <flux:menu.item :href="route('agencies.export.csv', request()->query())" icon="arrow-down-tray">
                            Export CSV
                        </flux:menu.item>
                        <flux:menu.item :href="route('agencies.export.xlsx', request()->query())" icon="document-arrow-down">
                            Export Excel
                        </flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
            ...
            @can('print agencies')
                <flux:dropdown position="bottom" align="end">
                    ...
                    <flux:menu>
                        <flux:menu.item :href="route('agencies.print', request()->query())" target="_blank" icon="printer">
                            Print View
                        </flux:menu.item>
                        <flux:menu.item :href="route('agencies.pdf', request()->query())" target="_blank" icon="document-text">
                            Download PDF
                        </flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
            ...
            <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <flux:text class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        Refine the agency list by company details, contact information, status, and page size.
                    </flux:text>
                </div>

                <div class="text-xs text-zinc-500 dark:text-zinc-400">
                    Showing {{ $this->agencies->firstItem() ?? 0 }}–{{ $this->agencies->lastItem() ?? 0 }}
                    of {{ $this->agencies->total() }} agencies
                </div>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
                <flux:input
                    ...
                    label="Search"
                    placeholder="Name, code, email, phone, contact"
                />
    ...
    <div class="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
            <flux:heading size="sm">Agency Directory</flux:heading>
            <flux:text class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                View, sort, and manage registered agency records.
            </flux:text>
        </div>
            ...
            <table class="min-w-full">
                ...
                <tr class="border-b border-zinc-200 dark:border-zinc-800">
                    <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                        <button wire:click="sortBy('name')" class="inline-flex items-center gap-1.5">Agency</button>
                    </th>
                    <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                ...
                @forelse ($this->agencies as $agency)
                    <tr class="transition hover:bg-zinc-50/80 dark:hover:bg-zinc-800/40">
                        <td class="px-5 py-4">
                            <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $agency->name }}</div>
                            ...
                            <div class="flex flex-wrap justify-end gap-2">
                                @can('toggle agencies')
                                    <flux:button
                                        ...
                                        wire:confirm="Are you sure you want to change this agency's status?"
                                        icon="{{ $agency->is_active ? 'pause-circle' : 'play-circle' }}"
                                    >
