<div class="space-y-8">
    <div class="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div class="space-y-2">
            ...
            <div>
                ...
                <flux:text class="mt-2 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                    Manage ship records, operational status, exports, and print-ready vessel reports.
                </flux:text>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            @can('create ships')
                <flux:button :href="route('ships.create')" wire:navigate icon="plus">
                    Add Ship
                </flux:button>
            @endcan
            ...
            @can('export ships')
                <flux:dropdown position="bottom" align="end">
                    ...
                    <flux:menu>
                        <flux:menu.item :href="route('ships.export.csv', request()->query())" icon="arrow-down-tray">
                            Export CSV
                        </flux:menu.item>
                        <flux:menu.item :href="route('ships.export.xlsx', request()->query())" icon="document-arrow-down">
                            Export Excel
                        </flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
            ...
            @can('print ships')
                <flux:dropdown position="bottom" align="end">
                    ...
                    <flux:menu>
                        <flux:menu.item :href="route('ships.print', request()->query())" target="_blank" icon="printer">
                            Print View
                        </flux:menu.item>
                        <flux:menu.item :href="route('ships.pdf', request()->query())" target="_blank" icon="document-text">
                            Download PDF
                        </flux:menu.item>
                    </flux:menu>
                </flux:dropdown>
            ...
            <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <flux:text class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        Refine the ship list by vessel details, type, status, and page size.
                    </flux:text>
                </div>

                <div class="text-xs text-zinc-500 dark:text-zinc-400">
                    Showing {{ $this->ships->firstItem() ?? 0 }}–{{ $this->ships->lastItem() ?? 0 }}
                    of {{ $this->ships->total() }} ships
                </div>
            </div>
        </div>
        ...
        <div class="p-5">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 2xl:grid-cols-5">
                <flux:input
                    ...
                    label="Search"
                    placeholder="Name, IMO, call sign, flag, owner"
                />

                <flux:select wire:model.live="type" label="Ship Type">
                    <option value="">All types</option>
                    @foreach ($this->shipTypes as $shipType)
    ...
    <div class="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
            <flux:heading size="sm">Ship Directory</flux:heading>
            <flux:text class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                View, sort, and manage vessel records.
            </flux:text>
        </div>
            ...
            <table class="min-w-full">
                ...
                <tr class="border-b border-zinc-200 dark:border-zinc-800">
                    <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                        <button wire:click="sortBy('name')" class="inline-flex items-center gap-1.5">Ship</button>
                    </th>
                    <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                        <button wire:click="sortBy('imo_number')" class="inline-flex items-center gap-1.5">IMO Number</button>
                    </th>
                    <th class="px-5 py-4 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                        Type
                    </th>
                ...
                @forelse ($this->ships as $ship)
                    <tr class="transition hover:bg-zinc-50/80 dark:hover:bg-zinc-800/40">
                        <td class="px-5 py-4">
                            <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $ship->name }}</div>
                            <div class="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                                {{ $ship->call_sign ?: 'No call sign' }}
                            </div>
                        </td>
                            ...
                            <div class="flex flex-wrap justify-end gap-2">
                                @can('toggle ships')
                                    <flux:button
                                        variant="ghost"
                                        size="sm"
                                        wire:click="toggleStatus({{ $ship->id }})"
                                        wire:confirm="Are you sure you want to change this ship's status?"
                                        icon="{{ $ship->is_active ? 'pause-circle' : 'play-circle' }}"
                                    >
                                ...
                                @can('delete ships')
                                    <flux:button
                                        ...
                                        wire:confirm="Are you sure you want to delete this ship?"
                                        icon="trash"
                                    >
                                        Delete
                                    </flux:button>
